...
Modular Spring Boot 4 backend foundation: verified Spring Modulith boundaries, JWT auth in the `iam` module, and enforced multi-tenant isolation driven by the authenticated JWT. This design is the architectural HOW; task breakdown and PR slicing belong to `sdd-tasks`.
...
| D2 | Multi-tenancy enforcement | Hibernate **`@TenantId`** (discriminator), NOT plain `@Filter`. Two tenant sources: the **subdomain** (`avicena.zendent.app`) for public requests, the **JWT `clinic_id`** (authoritative) for authenticated requests. |
...
| D4 | iam persistence | `clinic` (tenant root, global), `app_user` (global identity), `role` (global catalog), `membership` (tenant-scoped), `refresh_token`, `staff_invitation`. Flyway `V1__init.sql` baseline. |
...
- **Put tenancy/error handling in the base package instead of `shared`.** Rejected: they are reusable infrastructure that business modules must reference; an OPEN `shared` module is the idiomatic Modulith home and keeps the base package limited to bootstrap wiring.

---

## D2 — Multi-tenancy enforcement (the crux)

### Verdict: Hibernate `@TenantId`, not plain `@Filter`

Shared schema, discriminator column `clinic_id` on every tenant-scoped entity. Enforcement uses **Hibernate ORM discriminator multi-tenancy (`@TenantId`)** driven by a `CurrentTenantIdentifierResolver` that reads a request-scoped `TenantContext`.

### Why `@TenantId` over `@Filter` (rationale)
| Criterion | `@Filter` (`@FilterDef` + `session.enableFilter`) | `@TenantId` (chosen) |
|-----------|---------------------------------------------------|----------------------|
| Applies to `find()`/`getReference()` by id | **No** — direct primary-key loads bypass the filter → cross-tenant leak on `em.find(Patient, otherClinicId)` | **Yes** — Hibernate appends the tenant predicate to `find()` too |
| Enabled by default per session | **No** — must call `enableFilter` on every session or queries return ALL tenants silently | **Yes** — always active once the resolver returns a tenant |
| Write safety (INSERT) | **No** — filter is read-only; `clinic_id` must be set manually or rows land untenanted | **Yes** — Hibernate auto-populates the discriminator on persist |
...
### What is tenant-scoped vs global (critical nuance)
`@TenantId` requires a resolvable tenant for annotated entities. Authentication must work BEFORE a tenant is known, so identity tables are deliberately global:

| Entity | Tenant-scoped? | Reason |
|--------|----------------|--------|
| `Clinic` | No (it IS the tenant root) | Looked up by slug pre-auth; carries its own id, not a foreign `clinic_id` filter. |
| `User` (`app_user`) | No (global identity) | Email is globally unique; loaded by email at login before tenant context exists. |
| `Role` | No (global catalog) | System roles (ADMIN, DENTIST, STAFF...). |
| `Membership` | **Yes** (`@TenantId clinicId`) | The user↔clinic↔role join; "members of my clinic" must be tenant-isolated. |
| Future business entities (Patient, Appointment, Invoice...) | **Yes** | Inherit automatic isolation via `@TenantId`. |

Because login is **subdomain-scoped** (see D3 and the two-source precedence below — the clinic slug is derived from the request Host, not the body), the tenant context is established from the subdomain BEFORE `Membership` is touched, so `@TenantId` never blocks the auth flow. This removes the classic "cross-tenant lookup at login" problem without needing a bypass mechanism.

### Two tenant sources & precedence rule (explicit)
The tenant for a request comes from one of two sources, with a strict precedence:

| Request kind | Tenant source | Rule |
|--------------|---------------|------|
| **Public** (no JWT: `/auth/login`, `/auth/register` on a tenant subdomain, `/auth/refresh`, invitation accept) | **Subdomain** (Host header) | The subdomain is the SOLE tenant source. Slug → `Clinic` lookup → `TenantContext`. |
| **Authenticated** (valid JWT present) | **JWT `clinic_id`** — AUTHORITATIVE | The tenant comes from the token. The subdomain MUST MATCH the JWT `clinic_id`; a mismatch (a clinic-A token replayed on clinic-B's subdomain) MUST be rejected with **403**. |

The subdomain is derived SERVER-SIDE from the Host header — never sent as a client field — so it cannot be spoofed by a body parameter, and the JWT-vs-subdomain cross-check closes token-replay-across-tenants.
...
| `SubdomainTenantResolutionFilter` | `shared.tenancy` | **EARLY** (before/independent of JWT auth) | `OncePerRequestFilter`. Extracts the subdomain from the Host header; if it is a tenant subdomain, resolves `Clinic` by slug (global lookup) and calls `TenantContext.set(...)`. Skips apex/onboarding hosts (see D3). `clear()` in `finally`. This is what lets PUBLIC auth endpoints know the tenant before any JWT exists. |
...
| `TenantContextFilter` | `shared.tenancy` | **AFTER JWT auth** | `OncePerRequestFilter`. When authenticated, reads `clinic_id` from the JWT claims (AUTHORITATIVE), asserts it MATCHES the subdomain already resolved by `SubdomainTenantResolutionFilter` (else 403), and overwrites `TenantContext` with the JWT tenant. |

**Chain ordering:** `SubdomainTenantResolutionFilter` (early, sets tenant from Host) → JWT auth filter (resource-server, validates token) → `TenantContextFilter` (authoritative JWT tenant + subdomain match check). Public requests are fully tenant-scoped by the first filter alone.

### Sequence — request → auth → tenant set → filtered query
...
sequenceDiagram
    ...
    participant H as Hibernate + TenantResolver
    ...
    H->>DB: SELECT ... WHERE clinic_id = :tenant
...
`@ApplicationModuleTest` (or `@SpringBootTest` + Testcontainers): seed clinic A and clinic B each with a `Membership`; set `TenantContext` to A; assert repository returns ONLY A's memberships and that `find()` on B's membership id returns empty. This is the proposal's non-negotiable "clinic A cannot see clinic B" gate.

### Rejected alternatives
- **Plain `@Filter`** — rejected for the four failure modes in the table above.
- **Separate schema/database per tenant** — rejected: over-provisions for a dental-SaaS scale and contradicts the chosen shared-schema model; migration and connection management cost is not justified now.
...
sequenceDiagram
    ...
    S->>DB: find Membership(user, current tenant) [tenant-scoped]
...
| POST | `/auth/login` | tenant subdomain | public | Subdomain-scoped login (body `{email, password}`) → tokens. |
| POST | `/auth/refresh` | tenant subdomain | public (refresh token) | Rotate tokens. |
| POST | `/auth/logout` | tenant subdomain | bearer | Revoke refresh token. |
| POST | `/clinics/{id}/invitations` | tenant subdomain | bearer (ADMIN) | Staff invitation (tenant-scoped). |
| POST | `/invitations/{token}/accept` | tenant subdomain | public (invite token) | Accept invitation → create/link User + Membership. |
| GET | `/me` | tenant subdomain | bearer | Protected probe endpoint (proves JWT gate + subdomain match). |

### Onboarding & subdomain provisioning
Clinic onboarding is a chicken-and-egg case: there is no clinic (and thus no subdomain) yet, so `/auth/register` CANNOT be served from a tenant subdomain. It is served from a **root/apex host** (e.g. `app.zendent.app` or the apex `zendent.app`). `POST /auth/register` creates the `Clinic` with its globally-unique `slug`, which becomes the tenant's subdomain (`{slug}.zendent.app`).

**Apex-vs-subdomain distinction:** `SubdomainTenantResolutionFilter` classifies the Host against a configured base domain and an apex/reserved-label set (e.g. `app`, `www`, `api`, and the bare apex). If the label is reserved/apex → treat as a NON-tenant host, do NOT set `TenantContext`, and only apex-appropriate endpoints (`/auth/register`, health, docs) are reachable. If the label is a real slug → resolve the `Clinic` and set the tenant. An unknown, non-reserved subdomain that resolves to no clinic → **404** (unknown tenant).
...
- **`local`/`test` override:** `SubdomainTenantResolutionFilter` also honours a dev-only `X-Tenant-Slug` header (enabled ONLY under `local`/`test` profiles, never `prod`) so tooling and integration tests can set the tenant explicitly.
- **Testcontainers / `@ApplicationModuleTest`:** set the tenant via `TenantContext` directly or the `X-Tenant-Slug` header rather than real DNS. The mandatory isolation test drives tenant switching through the context/header, not the network.
...
```
Clinic (tenant root, global)
  1───* Membership *───1 User (global identity)
                    *───1 Role (global catalog)
User 1───* RefreshToken *───1 Clinic (session tenant)
...
|-------|-------------|-------|
| `clinic` | `id uuid pk`, `name`, `slug unique`, `status`, `created_at`, `updated_at` | Tenant root. No `clinic_id`. |
...
| `staff_invitation` | `id uuid pk`, `clinic_id fk NOT NULL`, `email`, `role_id fk`, `token unique`, `status`, `invited_by fk→app_user`, `expires_at`, `created_at`, `accepted_at null` | Tenant-scoped. Index `clinic_id`, `token`. |
...
`register` → INSERT `clinic` → INSERT/LINK `app_user` (admin) → `TenantContext.set(clinic.id)` → INSERT `membership(admin, clinic, ADMIN)` → publish `ClinicCreatedEvent(clinicId, ...)`. Invitation acceptance → create/link `app_user` → INSERT `membership` under the invitation's tenant → mark invitation ACCEPTED.
...
| Tenant base domain | `localhost` (`{slug}.localhost`) | `localhost` / `X-Tenant-Slug` | `zendent.app` |
...
- [ ] `SubdomainTenantResolutionFilter` (early) resolves tenant from Host; apex/reserved labels skipped; unknown slug → 404.
- [ ] `TenantContextFilter` (after JWT auth) applies authoritative JWT tenant and asserts subdomain == JWT `clinic_id` (mismatch → 403).
...
- [ ] Mandatory tenant-isolation test green (clinic A cannot read clinic B), driven via context/header.
